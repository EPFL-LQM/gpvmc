...
int main(int argc, char* argv[])
{
    ...
    if(comm_rank){
        ...
        const gsl_multimin_fdfminimizer_type *Tfdf=gsl_multimin_fdfminimizer_vector_bfgs2;
        const gsl_multimin_fminimizer_type *Tf=gsl_multimin_fminimizer_nmsimplex2;
        //gsl_multimin_fdfminimizer *s=NULL;
        gsl_multimin_fminimizer *s=NULL;
        gsl_vector *x,*ss;
        //gsl_multimin_function_fdf minexc_func;
        ...
        gsl_vector_set(ss,0,domap["step_phi"]);
        gsl_vector_set(ss,1,domap["step_neel"]);
        gsl_vector_set(ss,2,domap["step_hx"]);
        minexc_func.n=3;
        minexc_func.f=var_energy_f;
        //minexc_func.df=var_energy_df;
        //minexc_func.fdf=var_energy_fdf;
        minexc_func.params=&params;
        //s=gsl_multimin_fdfminimizer_alloc(Tfdf,3);
        //gsl_multimin_fdfminimizer_set(s,&minexc_func,x,0.01,1e-4);
        s=gsl_multimin_fminimizer_alloc(Tf,3);
        ...
        do {
            ...
            //status=gsl_multimin_test_gradient(s->gradient,domap["tolerance"]);
        ...
        fparamsout<<"phi="<<setprecision(6)<<gsl_vector_get(s->x,0)<<endl
...
double var_energy_f(const gsl_vector * x, void * params)
{
    ...
    MPI_Bcast(&count,1,MPI_INT,0,MPI_COMM_WORLD);
    double phi,neel,hx;
    phi=gsl_vector_get(x,0);
    neel=gsl_vector_get(x,1);
    hx=gsl_vector_get(x,2);
    cout<<"evaluation : "<<phi<<" "<<neel<<" "<<hx<<" = "<<flush;
    ...
    ++count;
    cout<<mean_energy<<" +/- "<<std_energy<<endl;
    return mean_energy;
}
