...
ProjHeis_1::ProjHeis_1(const Stepper_1* stepper,
                   FileManager* fm, const Lattice* lat,
                   double Bx)
    :MatrixQuantity_1(stepper,fm,"ProjHeis_1",
            2*stepper->GetAmp()->GetWaveFunction()->GetNExc(),
            stepper->GetAmp()->GetWaveFunction()->GetNExc()),
     m_lat(lat), m_Bx(Bx)
{}

void ProjHeis_1::measure()
{
#ifdef PROFILE
    Timer::tic("ProjHeis_1::measure");
#endif
    Quantity_1::measure();
    const Amplitude_1* amp=m_stepper->GetAmp();
    const LatticeState_1* st=amp->GetLatticeState();
    const WaveFunction_1* wav=amp->GetWaveFunction();
...
#ifdef PROFILE
    Timer::tic("ProjHeis_1::measure/VirtUpdate");
#endif
    amp->VirtUpdate(rhop,khop,qs);
    // qs has rhop.size() lines and khop.size() columns
#ifdef PROFILE
    Timer::toc("ProjHeis_1::measure/VirtUpdate");
#endif
...
#ifdef PROFILE
    Timer::toc("ProjHeis_1::measure");
#endif
}

std::string ProjHeis_1::str() const
{
