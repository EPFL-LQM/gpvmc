...
LatticeStepper_1::LatticeStepper_1(Amplitude_1* amp)
    :Stepper_1(amp),
...
BigDouble LatticeStepper_1::trystep()
{
#ifdef PROFILE
    Timer::tic("LatticeStepper::trystep");
#endif
    const WaveFunction_1* wav=m_amp->GetWaveFunction();
    const LatticeState_1* st=m_amp->GetLatticeState();
...
void LatticeStepper_1::step()
{
    LatticeState_1* st=m_amp->GetLatticeState();
    WaveFunction_1 *wav=m_amp->GetWaveFunction();
    st->Hop(m_prev);
    if(!m_khop<0){
...
BigDouble LatticeStepper_1::weight()
{
    const WaveFunction_1* wav=m_amp->GetWaveFunction();
    if(m_weight<0.0){
        BigDouble out(0);
...
double LatticeStepper_1::transprob()
{
    return 1.0;
}
